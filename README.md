...
`legacy-dc/` holds the original `.dc.html` (generated by the **Claude Design** visual editor) as a historical record — it's no longer edited or served; the React project in `src/` is the source of truth.
